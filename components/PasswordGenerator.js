...
export default class PasswordGenerator extends React.Component {
  constructor(props) {
    ...
    render() {
      const { label, password } = this.state;
      const { describePassword, describeLabel } = this.props;
      return (
        <View>
          <FormInput name="label" value={label} placeholder={describeLabel} onChanged={this.handleChange('label')} />
          <FormInput name="password" value={password} placeholder={describePassword} secureTextEntry onChanged={this.handleChange('password')} />
